...
function prakval(x) {
    $('[name="praktek1' + x + '"]').prop('readonly', true);
    $('[name="praktek2' + x + '"]').val("");
}
...
function nilaiKet(x) {
    ...
    var praktek1 = parseInt($('[name="praktek1' + x + '"]').val());
    var praktek2 = parseInt($('[name="praktek2' + x + '"]').val());
    var nk = (rtp + praktek1 + praktek2) / 3;

    $('[name="ket' + x + '"]').val(nk);
    var selisih_kkm = (100 - kkm) / 3;
    if (parseInt(nk) < kkm) {
        $('[name="predikat' + x + '"]').val("D");
        $('[name="deskrip' + x + '"]').val("Kurang Baik");
    } else if (parseInt(nk) >= kkm && parseInt(nk) <= Math.round(kkm + selisih_kkm) - 1) {
        $('[name="predikat' + x + '"]').val("C");
        $('[name="deskrip' + x + '"]').val("Cukup Baik");
    } else if (parseInt(nk) >= Math.round(kkm + selisih_kkm) && parseInt(nk) <= Math.round(kkm + (selisih_kkm * 2)) -
        1) {
        $('[name="predikat' + x + '"]').val("B");
        $('[name="deskrip' + x + '"]').val("Baik");
    } else if (parseInt(nk) >= Math.round(kkm + (selisih_kkm * 2)) && parseInt(nk) <= (kkm + (selisih_kkm *
            3))) {
        $('[name="predikat' + x + '"]').val("A");
        $('[name="deskrip' + x + '"]').val("Sangat Baik");
    }
}
                ...
                <table class="table table-bordered table-hover" style="table-layout:fixed;">
                    <thead>
                        <tr>
                            ...
                            <th colspan="7" class="text-center" width="550">NILAI KETERAMPILAN</th>
                            <th rowspan="2" class="text-center" width="95" style="line-height:55px;">RNT</th>
                            <th rowspan="2" class="text-center" width="95" style="line-height:55px;">PRAKTEK 1</th>
                            ...
                            <th rowspan="2" class="text-center" width="550" style="line-height:55px;">DESKRIPSI 2</th>
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td>
                                <input type="number" class="form-control" id="praktek1" name="praktek1<?=$row->idsiswa;?>" min="0"
                                    max="100" placeholder="0" value="<?=$nilai->nilai_praktek1;?>"
                                    onkeyup="this.value = minmax(this.value,0,100)"
                                    onchange="prakval(<?=$row->idsiswa;?>)"
                                    onclick="dropreadonly('praktek1<?=$row->idsiswa;?>')">
                            </td>
                            <td>
                                <input type="number" class="form-control" id="praktek2" name="praktek2<?=$row->idsiswa;?>" min="0"
                                    max="100" placeholder="0" value="<?=$nilai->nilai_praktek2;?>"
                                    onkeyup="this.value = minmax(this.value,0,100)"
                                    onchange="nilaiKet(<?=$row->idsiswa;?>)">
                            </td>
                            <td>
                                <input type="number" class="form-control" id="ket" name="ket<?=$row->idsiswa;?>"
                                    min="0" max="100" placeholder="0" value="<?=$nilai->nilai_ket;?>" readonly>
                            </td>
                            <td>
                                <input type="text" class="form-control" id="predikat" name="predikat<?=$row->idsiswa;?>"
                                    value="<?=$nilai->predikat_tp;?>" readonly>
                            </td>
                            <td>
                                <input type="text" class="form-control" id="deskrip"
                                    name="deskrip<?=$row->idsiswa;?>" value="<?=$nilai->deskripsi_tp;?>" readonly>
                            </td>
                            <td>
                                <input type="text" class="form-control" id="catatan"
                                    name="catatan<?=$row->idsiswa;?>" value="<?=$nilai->catatan_tp;?>">
                            </td>
                            <td>
                            ...
                            <td>
                                <input type="text" class="form-control" id="catatanp"
                                    name="catatanp<?=$row->idsiswa;?>" value="<?=$nilai->catatan_p;?>">
                            </td>
                        </tr>
                        <?php endforeach;?>
                        <?php endforeach;?>
