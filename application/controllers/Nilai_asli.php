    ...
    public function save($kelas_kd)
    {
        ...
        foreach (siswa_by_kelas(_active_years()->idtahun_akademik,_active_years()->semester,$idkelas) as $row){
            ...
            $data = [
                ...
                'nilai_praktek1'=>$this->input->post('praktek1'.$row->idsiswa,true),
                'nilai_praktek2'=>$this->input->post('praktek2'.$row->idsiswa,true),
                'nilai_ket'=>$this->input->post('ket'.$row->idsiswa,true),
                'predikat_tp'=>$this->input->post('predikat'.$row->idsiswa,true),
                'deskripsi_tp'=>$this->input->post('deskrip'.$row->idsiswa,true),
                'catatan_tp'=>$this->input->post('catatan'.$row->idsiswa,true),
                ...
                'catatan_p'=>$this->input->post('catatanp'.$row->idsiswa,true)
            ];
