...
* @return Object
*/
if (!function_exists('list_kelas_by_wali')){
	function list_kelas_by_wali($tahun_akademik,$semester,$idguru)
	{
		$CI =& get_instance();
		$wali_kelas = $CI->db->select('idwali_kelas')
		->where('idguru',$idguru)
		->get('wali_kelas')->row();
		$idwali_kelas = $wali_kelas->idwali_kelas;
		return $CI->db->join('kelas k', 'k.idkelas = n.idkelas', 'left')
		->where(['n.idtahun_akademik'=>$tahun_akademik,'n.semester'=>$semester,'n.idguru'=>$idguru])
		->get('wali_kelas n')->result();
...
/**
* Data Absensi By Siswa
* @param String
* @return Object
*/
if (!function_exists('absensi_by_siswa')){
	function absensi_by_siswa($tahun_akademik,$semester,$idkelas,$idsiswa)
	{
		$CI =& get_instance();
		return $CI->db->select('*')
		->where(['x.idtahun_akademik'=>$tahun_akademik,'x.semester'=>$semester,'x.idkelas'=>$idkelas,'x.idsiswa'=>$idsiswa])
		->get('absensi x')->result();
